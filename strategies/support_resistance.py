...
def backtest(df: pd.DataFrame, min_points: int, min_difference_points: int, rounding_nb: float, take_profit: float,
             stop_loss: float):
    ...
    for index, row in df.iterrows():
        for side in ['resistances', 'supports']:
            ...
            if row['rounded_' + h_l] in prices_group[side]:

                # group
                grp = prices_group[side][row['rounded_' + h_l]]
                ...
                if broken_in_last < 3 and (grp['last'] is None or index >= grp['last'] +
                                           min_difference_points * candle_length):
                ...
                if broken_in_last < 3:
                    ...
                    prices_group[side][row['rounded_' + h_l]] = {'prices': [row[h_l]], 'start_time': index,
                                                                 'last': index}
            ...
            for key, value in prices_group[side].items():
                if len(value['prices']) > 0:
                    ...
                    if side == "resistances" and row[h_l] > max(value['prices']):
                        ...
                        value['last'] = None
                    if side == "supports" and row[h_l] < min(value['prices']):
                        value['prices'].clear()
                        value['start_time'] = None
                        value['last'] = None
            ...
            for sup_res in resistances_supports[side]:
                ...
                entry_condition = row['close'] > sup_res['price'] if side == "resistances" else \
                    row['close'] < sup_res['price']
